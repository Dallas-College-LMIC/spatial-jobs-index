...
export class OccupationMapController extends BaseMapController {
  ...
  private async loadOccupationData(occupationId: string): Promise<void> {
    ...
    if (this.activeRequests.has(occupationId)) {
      console.log(
        `[OccupationController] Request already in progress for ${occupationId}, waiting...`
      );
    ...
    this.currentAbortController = this.apiService.createAbortController(
      `occupation-data-${occupationId}`
    );

    // Create new request and store it
    const requestPromise = this.loadOccupationDataDirect(
      occupationId,
  ...
  private async loadOccupationDataDirect(
    occupationId: string,
    signal?: AbortSignal
  ): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[OccupationController] Loaded ${occupationId} in ${loadTime}ms (cache: ${cacheHit ? 'HIT' : 'MISS'})`
      );
